<?php
    ...
    foreach ($data['ventas'] as $k) {
        $movimientos[] = array('id' => $k->id, 'tipo' => 'Venta', 'monto' => $k->monto, 'fecha' => $k->fecha);
    }
    foreach ($data['notas_credito'] as $k) {
        $movimientos[] = array('id' => $k->id, 'tipo' => 'Nota de crédito', 'monto' => $k->monto, 'fecha' => $k->fecha);
    }
    foreach ($data['cobros'] as $k) {
        $movimientos[] = array('id' => $k->id, 'tipo' => 'Cobro', 'monto' => $k->monto, 'fecha' => $k->fecha);
    }
