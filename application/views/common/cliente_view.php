...
$('#star1').starrr({
  change: function(e, value){
    if (value) {
        $('[name="puntaje"]').val(value); //alert(value);
    }
  }
});
</script>
...
function add_mensaje(clienteid)
{
    ...
    $.ajax({
        url : "<?php echo site_url('mensaje/ajax_list/')?>/" + clienteid,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            ...
            $.each(data, function(i, item) {
                resultado= resultado+'<div class="row mensaje mensaje'+item[0]+'"><label class="comentario">'+item[1]+'</label><label class="puntaje">'+item[2]+'</label><a href="javascript: delete_mensaje('+item[0]+')">X</a></div>';
...
function mensajes_cliente(id)
{
    //Ajax Load data from ajax
    $.ajax({
        url : "<?php echo site_url('cliente/ajax_listadoMensajes/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
...
function saveMensaje()
{
    $('#btnSave').text('saving...'); //change button text
    $('#btnSave').attr('disabled',true); //set button disable 
    var url = "<?php echo site_url('mensaje/ajax_add/')?>";
...
function delete_mensaje(id)
{
    if(confirm('Esta seguro que desea borrar este mensaje?'))
    {
        // ajax delete data to database
        $.ajax({
            url : "<?php echo site_url('mensaje/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
                ...
                <form action="#" id="form" class="form-horizontal">
                    ...
                    <div class="form-body">
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                                <input id="razon_social" name="razon_social" placeholder="Razon social" class="form-control" type="text" autofocus>
                    ...
                    <div class="form-body">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-9">
                                ...
                                <input id="puntaje" name="puntaje" placeholder="Puntaje" class="form-control" type="hidden">
                                <span class="help-block"></span>
                            </div>
                        </div>
                    </div>
